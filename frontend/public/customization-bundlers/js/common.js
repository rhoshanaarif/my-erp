    ...
    if ("undefined" != typeof OverlayScrollbars) {
      OverlayScrollbars(document.querySelectorAll(".scroll"), {
        ...
        document.querySelectorAll(".scroll-by-count").forEach((e) => {
          if ("undefined" == typeof ScrollbarByCount)
            return void console.log("ScrollbarByCount is undefined!");
    ...
    document
      ...
      .forEach((e) => {
        e.addEventListener("click", (e) => {
          ...
          let a = "";
          n.getAttribute("data-childSelector") &&
            (a = " " + n.getAttribute("data-childSelector")),
            (n.querySelector('[data-bs-toggle="dropdown"]' + a).innerHTML = o),
            n.querySelectorAll("a").forEach((e) => {
              e.classList.remove("active");
            }),
            "false" !== n.getAttribute("data-setActive") &&
              t.classList.add("active");
        });
      }),
      document.querySelectorAll(".dropdown-as-select").forEach((e) => {
        let t = "";
        e.getAttribute("data-childSelector") &&
          (t = " " + e.getAttribute("data-childSelector")),
          (e.querySelector('[data-bs-toggle="dropdown"]' + t).innerHTML =
            e.querySelector(".dropdown-menu a.active").textContent);
      });
  }
  _initModalPadding() {
      ...
      document.querySelectorAll(".modal").forEach((e) => {
        e.addEventListener("show.bs.modal", function (e) {
          ...
          e.addEventListener("hidden.bs.modal", function (e) {
            document.querySelector(".nav-container .nav-content") &&
              (document.querySelector(
  ...
  _setNotifyDefaults() {
    jQuery.notify &&
      jQuery.notifyDefaults({
        template:
          '<div data-notify="container" class="col-10 col-sm-6 col-xl-3 alert  alert-{0} " role="alert"><button type="button" aria-hidden="true" class="btn-close" data-notify="dismiss"></button><span data-notify="icon"></span> <span data-notify="title">{1}</span> <span data-notify="message">{2}</span><div class="progress" data-notify="progressbar"><div class="progress-bar progress-bar-{0}" role="progressbar" aria-valuenow="0" aria-valuemin="0" aria-valuemax="100" style="width: 0%;"></div></div><a href="{3}" target="{4}" data-notify="url"></a></div>',
      });
  }
  _setSelect2Defaults() {
    ...
    jQuery().validate &&
      (jQuery.validator.setDefaults({
        ...
        errorPlacement: function (e, t) {
          if (
            (-1 != t.attr("class").indexOf("form-check-input")
              ? e.insertAfter(t.parent())
              : e.insertAfter(t),
            t.parents(".tooltip-label-end").length > 0)
          )
            if (t.parents(".form-group").find(".form-label").length > 0) {
              let o =
                Math.round(
                  t.parents(".form-group").find(".form-label").width()
                ) + 10;
              jQuery(e).css("left", o);
            } else {
              let o =
                Math.round(t.parents(".form-group").find("label").width()) + 40;
              jQuery(e).css("left", o);
            }
        },
      }),
